...
@section('container')
@include('iklan.create')


<div class="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700 mt-14">
   <h1 class="text-5xl font-bold">Iklan</h1>
   <hr class="mt-2 mb-6">
   <div class="border-filter">
    <div class="flex items-center py-4 bg-white dark:bg-gray-800 justify-end mx-auto">
            <!-- Modal toggle -->
            <button class="inline-flex items-center text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-1 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" data-modal-target="create-iklan" data-modal-toggle="create-iklan" type="button">
                Create
                ...
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                    ...
                    <td class="px-6 py-4">
                        <div style="max-height: 250px; max-width: 400px; overflow:hidden;">
                            <img src="{{$row->iklan}}"
                            alt="{{ $row->link }}" class="img-fluid mt-3">
                        </div>
                    </td>
                    <td class="px-6 py-4">
                        {{$row->link}}
                    ...
                    <td class="px-6 py-4 flex">
                        <form action="{{ route('iklan.destroy',$row->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
                        <a href="#" class="font-medium text-blue-600 dark:text-blue-500 hover:underline" data-modal-target="edit-iklan-{{$row->id}}" data-modal-toggle="edit-iklan-{{$row->id}}">
